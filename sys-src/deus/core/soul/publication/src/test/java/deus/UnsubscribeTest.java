...
public class UnsubscribeTest extends AbstractUseCaseTest {
	...
	public void testUnsubscribeGrantedSubscription() {
		...
		// SubscribedProfileDeletedNotice notice =
		// (SubscribedProfileDeletedNotice)alice.getBarker().getUnnoticedAttentionList().get(0);
	...
	// FIXME: test is not working, since informationConsumer is not in LoS on
	// publisher side and thus cannot be removed
	/**
	...
	public void testUnsubscribeRequestedSubscription() {
		/*
		 * bob.getListOfPublishers().put(alice.getUserMetadata(),
		 * SubscriberSideSubscriptionState.requested);
		 * 
		 * testAttentionList(bob, 0, 0); testAttentionList(alice, 0, 0);
		 * bob.getSubscriber().unsubscribe(alice.getUserMetadata());
		 * 
		 * testAttentionList(bob, 0, 0); testAttentionList(alice, 1, 0);
		 * SubscriptionCanceledNotice notice =
		 * (SubscriptionCanceledNotice)alice.
		 * getBarker().getUnnoticedAttentionList().get(0);
		 * testDateNow(notice.getCreationDate());
		 * assertEquals(bob.getUserMetadata(), notice.getSubscriberMetadata());
		 * 
		 * alice.getBarker().noticeAttentionElement(notice);
