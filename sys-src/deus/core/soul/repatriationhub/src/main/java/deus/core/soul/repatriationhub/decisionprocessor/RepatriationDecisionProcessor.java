 ...
 */
@Named
public class RepatriationDecisionProcessor extends
		AbstractGenericDecisionProcessor<Repatriation> {
	...
	protected void processImpl(final UserId userId,
			final Repatriation contributionDecision) {
		if (contributionDecision.isDecisionPositive()) {
			this.pifGovernor.assimilateRepatriatedDigitalCard(
					new RepatriationAuthorityId(userId),
					contributionDecision.getContributedDigitalCard());
